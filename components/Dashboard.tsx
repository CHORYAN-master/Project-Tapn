...
import { Plus, Edit3, Trash2, Film, Home, Star, Lock } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        {drafts.map((draft) => (
          ...
          >
            {/* 샘플 배지 */}
            {draft.isSample && (
              <div className="absolute top-3 left-3 z-10 bg-gradient-to-r from-yellow-500 to-orange-500 text-white text-xs font-black px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1.5">
            ...
            {draft.isLocked ? (
              <div className="absolute top-2 right-2 p-2 bg-black/50 text-yellow-400 rounded-full opacity-0 group-hover:opacity-100">
                <Lock size={16} />
              </div>
            ) : (
